...
export class EntityGenerator {
	...
	public generateEntityInterface(metadata: EntityMetadata): string {
		...
		let code = `/**
 * Generated entity interface for ${entity} on ${new Date().toISOString()}
	...
	public generateRepositoryClass(metadata: EntityMetadata): string {
		...
		if (metadata.relations && metadata.relations.length > 0) {
			metadata.relations.forEach(relation => {
				...
				imports.add(relation.sourceEntity);
			});
		}
		imports.delete(entity);

		let code = `/**
 * Generated repository for ${entity} on ${new Date().toISOString()}
 * DO NOT EDIT THIS FILE DIRECTLY - it is generated from YAML metadata
 */

import { EntityDao } from "../entity/entity-manager.js";
...
${Array.from(imports).map(relation => `import { ${relation} } from "./${relation.toLowerCase()}.js";`).join('\n')}
...
export class ${entity}Repository extends EntityDao<${entity}> {
    ...
    return super.delete(id);
		...
		if (metadata.relations && metadata.relations.length > 0) {
			for (const relation of metadata.relations) {
				...
				const returnArray = relation.type === 'oneToMany' || relation.type === 'manyToMany';
				const returnType = returnArray
  ...
  async ${methodName}(id: number): Promise<${returnType}> {
	const related${relation.targetEntity} = await this.findRelated(id, "${relation.name}");
	return related${relation.targetEntity}.length > 0 ? related${relation.targetEntity}${returnArray ? '' : '[0]'} as ${returnType} : ${returnArray ? '[]' : 'undefined'};
  }`;
			}
		}
