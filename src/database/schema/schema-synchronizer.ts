		...
		try {
			...
			for (const column of columnsWithForeignKeys) {
				...
				try {
					...
					this.logger.error(new Error(`Error adding foreign key for column ${column.logical}: ${error.message}`, { cause: error }));
					// Continue with other foreign keys even if one fails
				}
			}
		} catch (error: any) {
			this.logger.error(new Error(`Error adding foreign keys to table ${tableName}: ${error.message}`, { cause: error }));
